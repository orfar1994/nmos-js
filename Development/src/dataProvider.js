    ...
    switch (type) {
        ...
        case UPDATE: {
            if (resource === 'devices') {
                ...
                for (const d of differences) {
                    let output_channel_index = get(
                        params.data.$staged.active,
                        `${d.path[0]}.${d.path[1]}.channel_index`
                    );
                    let output_channel_input = get(
                        params.data.$staged.active,
                        `${d.path[0]}.${d.path[1]}.input`
                    );
                    if (isNumber(output_channel_index)) {
                        output_channel_index = Number(output_channel_index);
                    }
                    let output_channel_map = {
                        channel_index: output_channel_index,
                        input: output_channel_input,
                    };
                    set(
                        patchData.action,
                        `${d.path[0]}.${d.path[1]}`,
...
const getCMEndPoints = (
    addresses,
    endpoints = ['io', 'map/active', 'map/activations']
) => {
    // Looking for the first promise to succeed or all to fail
    const invertPromise = p => new Promise((res, rej) => p.then(rej, res));
    const firstOf = ps => invertPromise(Promise.all(ps.map(invertPromise)));
